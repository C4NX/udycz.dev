<template>
  <div v-if="props.showTags ?? true">
    <div class="flex gap-4 items-center flex-wrap p-4 mb-4 bg-base-200 rounded-lg">
      <h2 class="font-bold">{{ $t('project.tags') }} :</h2>
      <button
        ...
        v-for="tag in allTags" :key="tag.name" @click="toggleTag(tag.name)">
        <Icon :name="tag.icon" />
        {{ tag.name }}
      </button>
    </div>
  </div>
  <p v-if="displayedItems.length === 0" class="text-center font-thin my-10 italic">{{ $t('project.empty') }}</p>
  <div v-else class="flex flex-wrap justify-between gap-10 px-5">
    <ProjectCard v-for="item in displayedItems" :key="item.id" :item="item" />
  </div>
  <h2 v-if="showLimit && undisplayedCount > 0" class="my-10">
